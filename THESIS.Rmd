---
output:
  pdf_document:
    fig_caption: yes
    number_sections: yes
    latex_engine: xelatex
    dev: png
documentclass: report
classoption: a4paper,oneside
bibliography: thesis.bib
csl: harvard-cite-them-right.csl
urlcolor: blue
linkcolor: blue
fontsize: 12pt
geometry: top = 1.8cm, bottom = 1.8cm, left = 1.5cm, right = 1.5cm, margin = 2.5cm
header-includes:
- \usepackage[section]{placeins}
- \usepackage{caption}
- \usepackage{fancyhdr} #for headers/footers
- \usepackage{setspace} #for 1.5 spacing
- \usepackage{chngcntr}
- \usepackage[export]{adjustbox}
- \usepackage{microtype}
- \usepackage{booktabs} #tables
- \usepackage{longtable}
- \usepackage{pdflscape}
- \usepackage{graphicx} #figures
- \usepackage{epigraph} #nice quotations
- \usepackage{pdfpages} #appendices
- \usepackage[inline]{enumitem} #no item seperate space
- \usepackage[flushleft]{threeparttable} #table footer
- \usepackage{hyperref} #cross-refs and hyperlinks
- \usepackage[toc,page]{appendix} #appendix in toc
#- \usepackage{fontspec} #to specify downloaded open fonts
#- \setmainfont{Junicode} also needs xelatex engine
- \usepackage{acronym}
- \usepackage[utf8]{inputenc} 
- \onehalfspacing
- \colorlet{Blue}{blue} #Sort out an issue with xcolor package(not explicitly loaded here! ok yeah)
---


```{r global_options, include=FALSE}
knitr::opts_chunk$set(fig.path = 'figures/', echo = FALSE,
                      warning = FALSE, message = FALSE)
knitr::opts_knit$set(base.dir = 'chapters/')
```

\pagenumbering{gobble}

```{r child = 'titlepage.Rmd'}
```

\newpage
\pagestyle{fancy}

\fancyhead[LE,RO]{}
\fancyhead[LO,RE]{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}

\pagenumbering{roman}

\section*{Abstract}
\addcontentsline{toc}{section}{Abstract}

```{r child = 'abstract.Rmd'}
```

\newpage
\fancyhead[CO,CE]{Table of Contents}
\setcounter{tocdepth}{2}
\tableofcontents

\newpage
\fancyhead[CO,CE]{List of Tables}
\addcontentsline{toc}{section}{List of Tables}
\listoftables

\newpage
\fancyhead[CO,CE]{List of Figures}
\addcontentsline{toc}{section}{List of Figures}
\listoffigures

\newpage
\section*{Acknowledgements}
\fancyhead[CO,CE]{}
\renewcommand{\headrulewidth}{0pt}
\addcontentsline{toc}{section}{Acknowledgements}

```{r child = 'acknowledgements.Rmd'}
```

\newpage
\section*{Declaration}
\fancyhead[CO,CE]{}
\renewcommand{\headrulewidth}{0pt}
\addcontentsline{toc}{section}{Declaration}

```{r child = 'declaration.Rmd'}
```

\newpage
\fancyhead[CO,CE]{Acronyms}
\addcontentsline{toc}{section}{List of Acronyms}
\vspace{2cm}
\section*{\huge{List of Acronyms}}\label{acros}
\vspace{1cm}
\input{acro_list}


\newpage
\pagenumbering{arabic}

```{r sans_serif, eval=FALSE}
#Remove chunk for sanserif font
\renewcommand{\familydefault}{\sfdefault}
```


\newpage
\fancyhf{}
\fancyhead[CE,CO]{\footnotesize \leftmark}
\fancyfoot[LE,LO]{\scriptsize \rightmark}
\fancyfoot[RE,RO]{\scriptsize Page \thepage}
\renewcommand{\headrulewidth}{0pt}

```{r child = 'intro.Rmd'}
```

\FloatBarrier
\newpage


```{r child = 'lit_review.Rmd'}
```


\FloatBarrier
\newpage

```{r child = 'data.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'renfrew.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'methods.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'results-1.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'results-2.Rmd'}
```

\FloatBarrier
\newpage
```{r child = 'results-3.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'discussion.Rmd'}
```

\FloatBarrier
\newpage

```{r child = 'conclusion.Rmd'}
```


\FloatBarrier
\newpage


```{r child = 'appendix_1.Rmd'}
```

\FloatBarrier
\clearpage

# References
